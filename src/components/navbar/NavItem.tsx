        ...
        <div className="flex">
            ...
            <div
                className={cn("absolute inset x-0 top-full text-sm text-muted-foreground",
                    {
                        "animte-in fade-in-10 slide-in-from-top-5" :
                            !isAnyOpen,
                    }
            )}>
                        ...
                        {category.featured.map((item) => (
                            <div key={item.name} className="group relative text-base sm:text-sm">
                                <div className="relative aspect-video overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75">
                                    <Image src={item.imageSrc} alt="produc category image" fill className="object-cover object-center" />
                                </div>

                                <Link href={item.href} className="mt-6 block font-medium text-gray-900">
                                <p className="mt-1"
                                aria-hidden="true">Shop now</p></Link>
                            </div>
                        ))}
                    </div>
                </div>
